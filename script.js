...
/* ════ HTML TEMPLATE — stored for offline export ════ */
const _TOP = `<!DOCTYPE html>
...
/* ════ EXPORT — يعمل بدون إنترنت على جميع الأجهزة ════ */
async function exportFile(){
  showToast('⏳ جاري تحضير الملف...');
  try {
    const gJson  = JSON.stringify(G);
    ...
    // بناء الملف كاملاً من الـ template المخزّن
    const bakedScript = `<script id="__baked__">
window.__BAKED_G__=${gJson};
window.__BAKED_IMG__=${imgJson};
<\/script>`;

    const html = _TOP + '<script>\n' + _BOT + '\n<\/script>\n' + bakedScript + '\n</body>\n</html>';
    ...
    if(isIOS && navigator.share){
      ...
      try {
        ...
        showToast('✅ اختر "حفظ في الملفات" من القائمة');
        return;
      } catch(shareErr){ /* المستخدم ألغى */ }
    }

    /* ── File System Access API (Android Chrome / Desktop) ── */
    if('showSaveFilePicker' in window){
      try {
        const fh = await window.showSaveFilePicker({
          suggestedName: fname,
    ...
    document.body.removeChild(a);
    setTimeout(()=>URL.revokeObjectURL(url), 3000);
    showToast('✅ تم التنزيل — تحقق من مجلد التنزيلات');

  } catch(err){
    console.error('export:', err);
    showToast('❌ حدث خطأ — تأكد من أن الملف محفوظ على جهازك');
  }
}
